...
error = [] # Error between the true and filtered positions
...
for i in range(1, n_iters):
    ...
    # Calculate the error
    error.append(np.linalg.norm(pos - est.flatten()))

    # Plot the results
    plt.clf()
    plt.plot(positions[:, 0], positions[:, 1], 'k', lw=3, label='True') # True position
